...
def compute_distance_partners(
    ...
    for zw in zw_list:
        ...
        for ticker in dist.columns:
            p[ticker] = dist[ticker].drop(ticker).nsmallest(5).index.tolist()
        partners[zw] = p
    return partners
...
def run_sleeve_for_spec(
    ...
    builder = builder.fully_invest().scale_risk(fn=make_equity_curve_regime_scale(scaler_start))
...
def estimate_weights_equal_sharpe(
    ...
    if pos_sharpes.empty:
        raise Exception("Unable to compute Sharpe ratio.")
